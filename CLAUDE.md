...
- **Colab動画制作** (`/colab-video`): ローカルにCUDA GPUが無くMiniMax H3を実行できない環境で、動画生成工程だけをGoogle Colabで実行するスキル。ユーザーから「**Colabで動画を作って**」「H3をColabで回して」と指示されたときに使う。**素材（キーフレーム・音声・プロンプト）は動画スキルで制作済みのランから受け取るのが既定**（seedanceランをH3チャプターへ変換する手順はSKILL内7章）。最終結合はローカルのまま、H3生成のみ同梱ノートブック（`h3_colab.ipynb`）でColabに切り出す。無料T4は配管検証用、本番生成はL4（Pay As You Go / Colab Pro）。**実行できるのはClaude CodeとCursorのみ（Codex対象外、`local-video`と同じ扱い）。** 詳細: `.claude/skills/colab-video/SKILL.md`
