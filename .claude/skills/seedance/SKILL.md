...
- **シードを`script.md`に記録する**: `dt_generate.sh`は使用したモデル名とシードを出力する。採用したキーフレームのシードを`script.md`の生成メモ（例: `## Keyframe generation notes`）に記録しておくと、再生成・微修正時に同じ絵を再現できる。フレームごとに「シード・入力画像・そのフレームで変えた点」を1行で残すと、後から破綻を追いやすい。
- **生成スクリプトそのものをラン専用ディレクトリに同梱してコミットする。** キーフレームのプロンプトは長く、拘束文の積み重ねが成果物の品質そのものなので、`script.md`のMotion promptだけでは再現できない。ランに`gen_keyframes.sh`（キーフレーム）と`gen_audio.sh`（音声）を置き、**リポジトリルートから実行できる相対パスで書き、既存の出力はスキップする**作りにする。これで次の3つが同時に満たせる:
  - 中断・再開ができる（1枚8分以上かかるので必須）
  - 特定のフレームだけ作り直せる（消して再実行するだけ）
  - 後任者が「どの拘束文でこの絵が出たのか」を読める（スキルの知見がランごとに実例として残る）
