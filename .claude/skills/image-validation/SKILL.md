...
0. **キャラの見た目の正はキャラクターシート**。`*_sheet.png`をReadで開かずに行った同一性チェックは無効とする。**キャラ同一性のFAILはブロッキング**で、1件でも残っている間は動画生成工程へ進ませない（`/seedance`ステップ3.5、`/local-video`ステップ6）。
...
4. **このスキルは1枚も再生成しない**。出力は検証レポートと修正リスト（`fix_list.md`）まで。再生成は生成元スキルのルールに従う（`/local-video`ステップ6-3、`/seedance`ステップ3・3.5）。修正リストが完全確定する前に部分的な作り直しを始めることを禁止する（チェーン崩壊と手戻りの温床）。
...
3. 登場キャラごとに`02_CHARACTERS/0N_*.md`の**NG変更**と**シート照合チェックリスト**を読み、**そのキャラの`*_sheet.png`をReadで開いてから**検証に入る（省略禁止・このスキルの前提）。シートを開かずに同定句や記憶だけで判定すると、`a white mask with red markings`のような要約に画像を寄せて読んでしまい、目穴・口のスリット・マーキング本数が全部違っていてもPASSにしてしまう（実測で発生済み）。
...
3. **キャラ同一性（シート照合）**: 登場キャラ全員について、各キャラ設定mdの「シート照合チェックリスト」を**1項目1行**に展開する（「NG要素があるか」の1行にまとめない）。「形が崩れる」と「丸ごと消える」の両方を見る。頭身・スケール（チビキャラと人間キャラの身長差）も含む。特に次の実測崩れは毎フレーム明示的に問う:
   - 仮面キャラ: 目穴が**黒い穴**のままか（人間の目・まぶた・まつげが見えていないか）、口の**スリット**が残っているか、赤いマーキングの**本数と左右対称性**が正典どおりか。
   - 後ろ姿・横顔・遠景のフレーム: 後頭部の髪・触手・楽器等のNG要素が消えていないか（正面フレームだけで判定しない）。
   - 体型・肌の色・衣装: 巨漢が細身になっていないか、グレーの肌が肌色に戻っていないか、白Tシャツがスーツ等へ入れ替わっていないか、他キャラの装備（ストラップ・名札）が移っていないか。
   - 参照画像のロゴ・文字が**顔や衣服へ焼き込まれていないか**（ロゴ参照を渡したランで実測）。
...
- 修正リスト確定後の再生成・追従・再検証は生成元スキルのルールに従う（`/local-video`ステップ6-3、`/seedance`ステップ3・3.5の再生成ルール）。再生成されたフレームと追従フレームは、本スキルのステップ2〜5を該当フレームだけ再実行して閉じる。
