...
| | L4 | A100 40GB |
|---|---|---|
| 速度 | 動画1秒≒4.5分（実測81秒/step） | **実測36.5秒/step＝L4比約2.2倍速**（2026-08・R2V 20step・WORKERS=2） |
...
- **高速化フラグ**: SageAttention（両GPU対象）と`--fast`系（L4のみ有効）をセル1で選べる。**A100は計測済みで結論が出ている: SageAttentionは効果なし（x0.95）→ A100はフラグなしが正**。L4は未計測のため、L4常用を検討するときに5章「高速化フラグのA/B計測」でパイロット計測すること。
...
| 設定 | 内容 | 効くGPU | 実測 |
|---|---|---|---|
| `SAGE_ATTENTION=True` | INT8量子化attention（pip `sageattention`＋`--use-sage-attention`。セル6がインストールと付与を行う） | L4 / A100 両方が対象 | **A100: 効果なし・不採用**（2026-08、base 36.5s/step vs sage 38.6s/step＝x0.95。H3はcomfy-aimdoのint8カーネルが既に効いており上乗せがないと推定）。L4未計測 |
| `FAST_FLAGS=["fp16_accumulation", "fp8_matrix_mult"]` | ComfyUIの`--fast`最適化 | **L4（Ada）のみ**（fp8行列積はAda/Hopper専用、fp16蓄積はA100だと等速。A100ではセル1が警告を出す） | L4未計測 |

**A100の本番はフラグなし（セル1の既定のまま）で確定**（2026-08計測）。残る高速化余地はstep数削減（品質A/B前提）・セリフなしチャプターのLTX-2.5移行・複数セッション並列。
