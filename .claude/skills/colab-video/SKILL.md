...
- **高速化フラグ**: SageAttention（両GPU対象）と`--fast`系（L4のみ有効）をセル1で選べる。倍率未実測のため、常用を決める前に5章「高速化フラグのA/B計測」でパイロット計測すること。
...
総時間の大半はサンプリング（L4実測81秒/step×20step）なので、1stepを安くする起動フラグを2系統用意してある。**倍率は未実測。常用を決める前にパイロットチャプター1本でA/Bを取り、実測をランの`H3_COLAB.md`に記録する**:

| 設定 | 内容 | 効くGPU |
|---|---|---|
| `SAGE_ATTENTION=True` | INT8量子化attention（pip `sageattention`＋`--use-sage-attention`。セル6がインストールと付与を行う） | L4 / A100 両方 |
| `FAST_FLAGS=["fp16_accumulation", "fp8_matrix_mult"]` | ComfyUIの`--fast`最適化 | **L4（Ada）のみ**（fp8行列積はAda/Hopper専用、fp16蓄積はA100だと等速。A100ではセル1が警告を出す） |

A/Bの手順（1条件あたりチャプター1本。`AUTO_SHUTDOWN=False`にしておく）:

1. `AB_LABEL="base"`・フラグ全OFFでパイロットチャプターを生成する。
2. `SAGE_ATTENTION=True`（L4なら`FAST_FLAGS`も）＋`AB_LABEL="sage"`に変えて**セル1→6→7を再実行**する（フラグはComfyUIの起動引数なので、セル6の再起動を挟まないと反映されない）。
3. セル7が`/content/outputs/bench_log.csv`へ「ラベル・チャプター・GPU・WORKERS・フラグ・wall秒・サンプリング1step秒」を毎チャプター追記する（`OUT_DRIVE_DIR`設定時はDriveにもコピーされ、次セッションはDrive側から引き継いで追記＝セッションを跨いで比較できる）。セル7の末尾にも全行が表示されるので、`base`と`sage`のwall秒/1step秒を比較する。
4. workflowが同一＝シードも同一なので、`ch*__base.mp4`と`ch*__sage.mp4`は条件差だけの比較になる。品質（リップシンク・キャラ同一性・音声・破綻）を見比べ、劣化がなければ採用。実測倍率を`H3_COLAB.md`と本スキル0章の速度目安に反映する。
5. 本番ランでは`AB_LABEL`を空に戻す（採用したフラグは付けたまま）。A/B成果物を本番の結合に流用するなら`chN.mp4`へリネームしてから回収する。
