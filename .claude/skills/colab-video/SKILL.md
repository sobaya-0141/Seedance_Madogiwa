...
- **高速化フラグ（両GPUとも計測済み・2026-08）**: **A100はフラグなしが正**（SageAttentionはx0.95で逆効果）。**L4は`SAGE_ATTENTION=True`が正**（x1.07。品質は採用前にパイロットで確認）。`--fast`系はどちらのGPUでも効果ゼロ（x1.00）＝常用しない。詳細は5章「高速化フラグのA/B計測」の実測表。
...
- **さらに推奨: GCS重みキャッシュ（セル1の`WEIGHTS_GCS_BUCKET`・2026-08導入。設定時はDriveより優先）**。Google Cloud Storageに重みを置くと`gcloud storage`の並列DLになり、Drive FUSE（83MB/s）より大幅に速い。**実測（2026-08・L4）: 接続から生成開始まで4.5分**（Drive運用時は約15分。52GBのGCS DLがpip install・バンドル展開・ComfyUI起動と完全に並走し、セル7開始時点で配置完了済みだった）。設定は`"auto"`（プロジェクトIDから`gs://<project-id>-h3-weights`を自動導出）でよい。初回配置は**セル0を`GCS_BUCKET="auto"`で実行**（無料CPUランタイム・バケット自動作成・**Driveに既にある重みはHFを介さずDrive→GCSへ直接コピー**）。認証はColab標準の`auth.authenticate_user()`（ポップアップ1回）で、失敗時はDrive/HFへ自動フォールバック。
  - **バケットは必ずシングルリージョン（セル0の`GCS_LOCATION`既定＝us-central1）にする。**「同一大陸内のGCPサービス向け転送は無料」はColab相手には成立しなかった（2026-08実測）: USマルチリージョンだと**書き込み時の内部複製費**（`Replication within Northern America`・117GBで¥216）と**読み出し課金**（`Multi-region within Northern America`・約$0.01/GB＝52GB/セッションで約¥80）が乗る。シングルリージョンなら複製費は存在せず、**Colabが同一リージョンに当たったランは読み出し無料**（外れても$0.01/GB）。バケットのロケーションは後から変更できないため、移行はセル0の`ALLOW_BUCKET_RECREATE=True`で中身ごと作り直す（Driveから再アップロード）。
  - **費用の目安（実測ベース）**: 保管us-central1 $0.020/GB/月＝117GBで月¥360前後（使わないGPUペアを消せば¥170〜230）＋読み出しは上記（期待値ほぼ¥0）。**ローカルMacへGCSから直接DLしない**（インターネット向け$0.12/GB。回収は従来どおりDrive経由）。
  - **バンドルzipと成果物の受け渡しはDriveのまま**（`BUNDLE_ZIP_FROM_DRIVE`/`OUT_DRIVE_DIR`は少容量で無料15GB枠に収まる＝重みをGCSへ移してDriveの`h3_weights`を消せばGoogle Oneは解約できる。ただしGmail/フォト込みで15GB以下が条件）。
...
| セル | 内容 |
|---|---|
| 0 | **（初回のみ・無料CPUランタイムで単独実行）重みのGCS/Drive事前配置**: 対象GPU（`TARGET_GPUS`）のunetペア＋共通重みを配置先（`GCS_BUCKET`設定時はGCS・バケット自動作成、未設定ならDrive）に揃える。**Driveに既にある重みはHFを介さずDrive→GCSへ直接コピー**、無い分はaria2でDL→正確なバイト数で完全性照合→アップロード。配置済み分はskipするので再実行は常に安全 |
| 1 | **設定＋環境チェック（毎セッションここだけ編集）**: `CHAPTERS`・`EXPECTED_GPU`・`WORKERS`・`AUTO_SHUTDOWN`・Driveパス・`NEED_I2V`/`NEED_R2V`・高速化フラグ（`SAGE_ATTENTION`・`FAST_FLAGS`）・A/B計測ラベル（`AB_LABEL`、下記「高速化フラグのA/B計測」）等。実行すると**実際に割り当てられているGPUを正として**重みバリアントを自動選択・表示し、`WORKERS`から各ワーカーのVRAM上限（`--reserve-vram`）を決める。`EXPECTED_GPU`と違えばここで停止 |
...
| 3 | **重み配置（バックグラウンド実行 — 開始したらすぐセル4〜6へ進んでよい。セル7の冒頭が完了を待つ）**: **GCS優先（`WEIGHTS_GCS_BUCKET`設定時。`gcloud storage`並列DL）**→Drive（8スレッド並列コピー。サイズ照合で不完全ファイル検出、コピーは中断してもレジューム。GCS運用中はDriveにしか無い重みを通りがかりにGCSへ複製）→無い分は**aria2（16並列・レジューム・進捗表示）**でDL→GCS（またはDrive）へ保存。**GPU切替時は容量が足りなければDrive上の旧バリアントを自動削除**。ユニットは**`NEED_I2V`/`NEED_R2V`の片方だけがTrue（＝使うユニットが1本）ならここで先にローカル化**し（共通33GB＋21GB≒54GBで収まる）、両方必要なランでは同時に載らないのでsymlinkに留めてセル7が遅延ローカル化する |
...
| 7 | **チャプター生成**: 冒頭でセル3のバックグラウンド重み配置の完了を待ち、**同じユニットを使うチャプターをグループにまとめ**（21GBのユニット入れ替え回数を最小化。同一グループ内は番号順）、グループのユニットを**流す直前にGCS/Driveからローカル化**（ディスクが足りなければ他方のユニットを自動退避。**削除した旧unetをComfyUIがmmapで掴んでいて空きが戻らないときは、自動でComfyUIを再起動して解放してからやり直す** — 2026-08実測）してから**`WORKERS`本ずつ同時実行**（出力は`[w0]`/`[w1]`のワーカー名付きで流れる）。生成済みスキップは**ローカルと`OUT_DRIVE_DIR`上のmp4の両方**を見る＝セッションを跨いだ再開でも退避済みチャプターを再生成しない。`OUT_DRIVE_DIR`設定時は**完了ごとに即Drive退避**。`AUTO_SHUTDOWN=True`なら全チャプター成功＋Drive退避を確認後に**ランタイムを自動削除** |
| 8 | 成果物の回収（zip→ブラウザDL） |
...
- **FUSE読み出しの並列化ベンチ（2026-08、未読領域4GiBずつをpreadで読み比べ）**: 1/4/8/16スレッド = 62.6 / 76.0 / 83.4 / 76.6 MB/s。8スレッドが最速（約1.33倍）だったのでコピーは8スレッド並列preadを採用。伸びが小さいのはFUSEよりDriveバックエンドが律速のため。→ **これを超えるため2026-08にGCS重みキャッシュを実装**（0章の`WEIGHTS_GCS_BUCKET`参照。`gcloud storage cp`の並列DLで数百MB/s級）。
