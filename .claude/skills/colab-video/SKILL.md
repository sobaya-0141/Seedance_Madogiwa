...
- **さらに推奨: GCS重みキャッシュ（セル1の`WEIGHTS_GCS_BUCKET`・2026-08導入。設定時はDriveより優先）**。Google Cloud Storageに重みを置くと`gcloud storage`の並列DLになり、Drive FUSE（83MB/s）より大幅に速い。費用は従量課金だが実質保管料のみ: USマルチリージョン約$0.026/GB/月（75GBで月¥300前後、117GBで¥460前後）、**Colab（US）への転送は同一大陸内のGCPサービス向けなので無料**。設定は`"auto"`（プロジェクトIDから`gs://<project-id>-h3-weights`を自動導出）でよい。初回配置は**セル0を`GCS_BUCKET="auto"`で実行**（無料CPUランタイム・バケット自動作成・**Driveに既にある重みはHFを介さずDrive→GCSへ直接コピー**）。認証はColab標準の`auth.authenticate_user()`（ポップアップ1回）で、失敗時はDrive/HFへ自動フォールバック。**バンドルzipと成果物の受け渡しはDriveのまま**（`BUNDLE_ZIP_FROM_DRIVE`/`OUT_DRIVE_DIR`は少容量で無料15GB枠に収まる＝重みをGCSへ移してDriveの`h3_weights`を消せばGoogle Oneは解約できる。ただしGmail/フォト込みで15GB以下が条件）。**ローカルMacへGCSから直接DLしない**（インターネット向け転送$0.12/GBが発生。回収は従来どおりDrive経由）。
